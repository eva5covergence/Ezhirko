...
def test(cfg,
    ...
    s = ('%20s' + '%17s' * 10) % ('Class', 'Images', 'Targets', 'P', 'R', 'mAP@0.5', 'F1', 'RmseGradientLoss', 'SSIMLoss', 'DLoss', 'TotalLoss')
    ...
    for batch_i, (imgs, targets, paths, shapes,depth_targets,_,_) in enumerate(tqdm(dataloader, desc=s)):
        ...
        with torch.no_grad():
            ...
            depth_out, (inf_out, train_out) = model(imgs)  # inference and training outputs
            ...
            # Compute loss
            if hasattr(model, 'hyp'):  # if model has loss hyperparameters
                bbox_loss += bbox_lambda * compute_loss(train_out, targets, model)[1][:3]  # GIoU, obj, cls
                rmse_gradient_mean_loss, ssim_mean_loss = [depth_lambda * item for item in compute_depth_loss(depth_out, depth_targets, batch_size, device)]
    ...
    print(pf % ('all', seen, nt.sum(), mp, mr, map, mf1, rmse_gradient_loss, ssim_loss, depth_loss, total_loss))
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return (mp, mr, map, mf1, *(bbox_loss.cpu() / len(dataloader)).tolist(), depth_loss/len(dataloader),total_loss/len(dataloader)), maps
