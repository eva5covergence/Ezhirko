...
class PlaneDatasetSingle(Dataset):
    def __init__(self, options, config, split, random=True, loadNeighborImage=False, load_semantics=False, load_boundary=False):
        ...
        scene = ScanNetScene(options, '', '', '', '', load_semantics=load_semantics, load_boundary=load_boundary)
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:
                ...
                image, planes,segmentation, depth, camera, extrinsics, semantic = scene[imageIndex]
                if len(planes) == 0:
                    index += 1                    
                    continue
            except:
