...
class ScanNetScene():
    ...
    def __init__(self, options, scenePath, scene_id, confident_labels, layout_labels, load_semantics, load_boundary=False):
        ...
        if self.scannetVersion == 1:
            ...
            cam_path = str(Path('data/customdata/ground_truth/camera.txt')) 
            assert os.path.isfile(cam_path), 'File not found %s.' % (cam_path)
            with open(cam_path) as f:
                for line in f:
                    line = line.strip()
                    tokens = [token for token in line.split(' ') if token.strip() != '']
                    self.camera[0] = tokens[0]
                    self.camera[1] = tokens[1]
                    self.camera[2] = tokens[2]
                    self.camera[3] = tokens[3]
                    self.colorWidth = tokens[4]
                    self.colorHeight = tokens[5]
                    self.depthWidth  = tokens[4]
                    self.depthHeight = tokens[5]
                pass
            self.depthShift = 1000.0
            img_path = 'data/customdata/inputs/interior_input_images/'
            self.imagePaths = glob.glob(img_path + '*.jpg')
            pass
            
        self.camera[4] = self.depthWidth
        self.camera[5] = self.depthHeight
        
        surf_path = 'data/customdata/ground_truth/planar_surface_gt_images/'
        self.planesPaths = glob.glob(surf_path + '*_plane_parameters.npy')
        #self.planes = np.load('PPEMultiModel/data/customdata/ground_truth/planar_surface_gt_images/' + '*_plane_parameters.npy')

        #self.plane_info = np.load(scenePath + '/annotation/plane_info.npy')    
        self.plane_info = self.planesPaths
        if len(self.plane_info) != len(self.planesPaths):
    ...
    def __getitem__(self, imageIndex):
        ...
        if self.scannetVersion == 1:
            ...
            segmentationPath = imagePath.replace('inputs/interior_input_images/', 'ground_truth/planar_surface_gt_images/').replace('.jpg', '_segmentation_final.png')
            depthPath = imagePath.replace('inputs/interior_input_images/', 'ground_truth/Midas_Interior/').replace('.jpg', '.png')
            posePath = './data/customdata/ground_truth/pose.txt'
            planes = np.load(imagePath.replace('inputs/interior_input_images/', 'ground_truth/planar_surface_gt_images/').replace('.jpg', '_plane_parameters.npy'))
        ...
        try:
            ...
            print('no depth image', depthPath, imagePath)
            exit(1)
        ...
        planes_mask = np.load(imagePath.replace('inputs/interior_input_images/', 'ground_truth/planar_surface_gt_images/').replace('.jpg', '_plane_masks.npy'))
        masks = np.concatenate([np.maximum(1 - planes_mask.sum(0, keepdims=True), 0), planes_mask], axis=0).transpose((1, 2, 0))
        segmentation = drawSegmentationImage(masks,blackIndex=0)
        ...
        depth = cv2.resize(depth, (segmentation.shape[1], segmentation.shape[0]))
