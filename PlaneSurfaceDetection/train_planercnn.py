    ...
    for epoch in range(options.numEpochs):
        ...
        for sampleIndex, sample in enumerate(data_iterator):
            ...
            for indexOffset in [0, 13]:
                ...
                print('gt_parameters',gt_parameters.shape)
                print('gt_depth',gt_depth.shape)
                print('extrinsics',extrinsics.shape)
                print('gt_plane',gt_plane.shape)
                print('gt_segmentation',gt_segmentation.shape)
                print('plane_indices',plane_indices.shape)
                if indexOffset == 13:
                ...
                print('rpn_pred_bbox',rpn_pred_bbox.shape)
                print('target_class_ids',target_class_ids.shape)
                print('mrcnn_class_logits',mrcnn_class_logits.shape)
                print('target_deltas',target_deltas.shape)
                print('mrcnn_bbox',mrcnn_bbox.shape)
                print('target_mask',target_mask.shape)
                print('mrcnn_mask',mrcnn_mask.shape)
                print('target_parameters',target_parameters.shape)
                print('mrcnn_parameters',mrcnn_parameters.shape)
                print('detections',detections.shape)
                print('detection_masks',detection_masks.shape)
                print('detection_gt_parameters',detection_gt_parameters.shape)
                print('detection_gt_masks',detection_gt_masks.shape)
                ...
                print('depth_np_pred',depth_np_pred.shape)
                ...
                if config.PREDICT_NORMAL_NP:
                    print('inside PREDICT_NORMAL_NP')
                ...
                if len(detections) > 0:
                    print('after:',depth_np_pred.shape)
